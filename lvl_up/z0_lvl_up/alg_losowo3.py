...
def get_next_index4(last_element_index, data_level, data):
    next1, next2 = data[data_level][last_element_index], data[data_level][last_element_index + 1]
    chance1, chance2 = 10-next1, 10-next2
    data_len = len(data)
    lvl_impact = (data_len - data_level) / data_len
    difference = chance1 - chance2
    final_difference = difference * lvl_impact
    chance2 += final_difference
    return choices(
        [last_element_index, last_element_index + 1],
        [chance1, chance2]
    )[0]
...
run(ts.medium, get_next_index4, 420)
